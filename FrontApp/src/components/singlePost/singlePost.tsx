...
const SinglePostPage: React.FC = () => {
  ...
  // Helper functions to format display name and date.
  ...
  const handleUpdatePost = async () => {
    console.log("Save clicked", editedTitle, editedContent);
    if (!post) return;
    try {
  ...
  // Handler for deleting the post and its comments.
  const handleDeletePost = async () => {
    if (!post) return;
    try {
      // Call deletePost API function
      await deletePost(post._id);
      // Optionally, if your backend does not cascade-delete comments,
      // you can iterate through comments and delete them individually.
      // For example:
      // for (const comment of comments) {
      //   await deleteComment(comment.id);
      // }
      // Navigate back to the feed after deletion.
      navigate("/feed");
    } catch (error) {
          ...
          >
            ...
            <CardContent sx={{ flex: 1, p: 4 }}>
              ...
              <Box sx={{ display: "flex", alignItems: "center", mb: 3 }}>
                <Avatar
                  sx={{ bgcolor: "#3f51b5", mr: 2, width: 42, height: 42 }}
                >
              ...
              {/* Edit Post and Delete Post buttons (only visible to post owner) */}
